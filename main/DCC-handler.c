...
static const char *TAG = "DCC-handler";
...
int espnow_data_parse(uint8_t *data, uint16_t data_len, uint8_t *state, uint16_t *seq, int *magic)
{
    ...
    *magic = buf->magic;
    ...
    // Print each byte of payload
    for (int i = 0; i < 9; i++) {
        printf("%02x ", buf->payload[i]);
    }

    // Latitude is float value in 4 bytes (1-5) in payload, longitude is float value in 4 bytes (6-9) in payload
    ...
    } else if (buf->payload[0] == 1) {
        printf("MOB: WAKE, %f, %f, %f\n", 0.0, longitude, latitude);
    } else if (buf->payload[0] == 2) {
        printf("MOB: RESET, %f, %f, %f\n", 0.0, longitude, latitude);
    } else {
        printf("MOB: , %f, %f, %f\n", 0.0, longitude, latitude);
    }
